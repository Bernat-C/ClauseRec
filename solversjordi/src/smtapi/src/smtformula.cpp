...
boolvar SMTFormula::trueVar(std::vector<float> *features){
	if(truevar.id==0){
		truevar=newBoolVar(*features);
		addClause(truevar);
	}
	return truevar;
}

boolvar SMTFormula::falseVar(std::vector<float> *features){
	if(falsevar.id==0){
		falsevar=newBoolVar(*features);
		addClause(!falsevar);
	}
	return falsevar;
}
...
boolvar SMTFormula::newBoolVar(){
	boolvar x;
	x.id=++nBoolVars;
	std::string s = auxboolvarpref+"__"+std::to_string(x.id);
	std::vector<float> features;
	varsFeatures.push_back(features);
...
void SMTFormula::addClause(const clause &c) {
	for(literal l: c.v) {
		if(l.v.id<0 || l.v.id>100000000) {
			std::cerr << "The clause " << l.v.id << " might be an error." << std::endl;
			assert(false); 
		}
	}
	clauses.push_back(c);
...
void SMTFormula::addAMO(const std::vector<literal> & x, AMOEncoding enc, std::vector<float> *features) {
	...
	switch(enc){
		...
		{
			if(n <= 3)
				addAMO(x,AMO_QUAD,features);
			else{
				//Recursively encode AMO(x1,x2,y) /\ AMO(x3,x4,...,xn,¬y)
				boolvar y = newBoolVar(*features);
				std::vector<literal> v1(x.begin(),x.begin()+2);
				std::vector<literal> v2(x.begin()+2,x.end());
				v1.push_back(y);
				v2.push_back(!y);
				addAMO(v1,AMO_QUAD,features); //AMO(x1,x2,y)
				addAMO(v2,AMO_LADDER,features); //AMO(x3,x4,...,xn,¬y)
			}
		}
			break;
		...
		case AMO_COMMANDER:
		{
			if(n<6)
				...
				if(nsplits==2){
					cmd_vars[0] = newBoolVar(*features);
					cmd_vars[1] = !cmd_vars[0];
				}
				else{
					for(int i= 0; i < nsplits; i++) {
						cmd_vars[i] = newBoolVar(*features);
					}
					addEO(cmd_vars,AMO_COMMANDER,features);
				}
...
void SMTFormula::addEO(const std::vector<literal> & v, AMOEncoding enc, std::vector<float> *features) {
	addALO(v);
	addAMO(v,enc,features);
}


void SMTFormula::addALK(const std::vector<literal> & v, int K, CardinalityEncoding enc, std::vector<float> *features){
	int n = v.size();
		...
		switch(enc){
			...
			default:
			{
				if(K > n/2){ //AMK encoding of the negated literals is smaller
					...
					for(const literal & l : v)
						v2.push_back(!l);
					addAMK(v2,n-K,CARD_SORTER,features);
				}
				else{
					std::vector<literal> sorted;
					addMCardinality(v,sorted,K,false,true,features);
...
void SMTFormula::addAMK(const std::vector<literal> & v, int K, CardinalityEncoding enc, std::vector<float> *features){
	int n = v.size();
	if(K < 0){ //Trivially false
		...
		switch(enc){
			case CARD_TOTALIZER:
			{
				...
				addTotalizer(v,root,K,features);
				addClause(!root[K]);
			}
			break;
			...
			case CARD_SC:
			{
				addAMKSequentialCounter(v,K,features);
			}
			break;
			...
			default:
			{
				if(K > n/2){ //ALK encoding of the negated literals is smaller
					...
					for(const literal & l : v)
						v2.push_back(!l);
					addALK(v2,n-K,smtapi::CARD_SORTER,features);
				}
				else{
					std::vector<literal> sorted;
					addMCardinality(v,sorted,K+1,true,false,features);
					addClause(!sorted[K]);
				}
			}
			break;
...
void SMTFormula::addEK(const std::vector<literal> & v, int K, std::vector<float> *features){
	int n = v.size();
	if(K>n | K < 0){ //Triviavlly false
		...
		if(K > n/2){ //EK encoding of the negated literals is smaller
			...
			for(const literal & l : v)
				v2.push_back(!l);
			addEK(v2,n-K,features);
		}
		else{
			std::vector<literal> sorted(n);
			addMCardinality(v,sorted,K+1,true,true,features);
			addClause(sorted[K-1]);
			addClause(!sorted[K]);
		}
	}
...
void SMTFormula::addPB(const std::vector<int> & Q, const std::vector<literal> & X, int K, PBEncoding encoding, std::vector<float> *features){
	...
	switch(encoding){
		...
		case PB_BDDIO:
		{
			...
			MDD * mdd = mb.getMDD();
			addClause(assertMDDLEQAbio(mdd,features));
		}
			break;

		case PB_SWC:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_GSWC,features);
			break;

		case PB_GT:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_GGT,features);
			break;

		case PB_RGT:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_RGGT,features);
			break;

		case PB_RGTnoR:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_RGGTnoR,features);
			break;

		case PB_RGTnoPre:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_RGGTnoPre,features);
			break;

		case PB_MTO:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_GMTO,features);
			break;


		case PB_LPW:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_GLPW,features);
			break;

		case PB_GPW:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_GGPW,features);
			break;

		case PB_LBM:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_GLBM,features);
			break;

		case PB_GBM:
			util::PBtoAMOPB(Q2,X2,QQ,XX);
			addAMOPB(QQ,XX,K,AMOPB_GGBM,features);
			break;
...
void SMTFormula::addAMOPB(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, AMOPBEncoding encoding, std::vector<float> *features){
	...
	if(it != amopb_pb_rel.end()){
		...
		addPB(Q2,X2,K,it->second,features);
		return;
	}
	...
	switch(encoding){
		...
		case AMOPB_GSWC:
			addAMOPBSWC(Q2,X2,K,features);
			break;

		case AMOPB_SORTER:
			addAMOPBSorter(Q2,X2,K,features);
			break;

		case AMOPB_GGT:
			addAMOPBGeneralizedTotalizer(Q2,X2,K,features);
			break;

		case AMOPB_RGGT:
		case AMOPB_RGGTnoPre:
			addAMOPBReducedGeneralizedGeneralizedTotalizer(Q2,X2,K,true,features);
			break;

		case AMOPB_RGGTnoR:
			addAMOPBReducedGeneralizedGeneralizedTotalizer(Q2,X2,K,false,features);
			break;

		case AMOPB_GMTO:
			addAMOPBModuloTotalizer(Q2,X2,K,features);
			break;


		case AMOPB_GGBM:
			addAMOPBGlobalPolynomialWatchdog(Q2,X2,K,true,features);
			break;

		case AMOPB_GLBM:
			addAMOPBNaiveLocalPolynomialWatchdog(Q2,X2,K,true,features);
			break;

		case AMOPB_GLPW:
			addAMOPBLocalPolynomialWatchdog(Q2,X2,K,features);
			break;

		case AMOPB_GGPW:
			addAMOPBGlobalPolynomialWatchdog(Q2,X2,K,false,features);
			break;
...
void SMTFormula::addPBGEQ(const std::vector<int> & Q, const std::vector<literal> & X, int K, PBEncoding encoding, std::vector<float> *features){
	...
	addPB(Q,X2,K,encoding,features);
}


void SMTFormula::addAMOPBGEQ(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, AMOPBEncoding encoding, std::vector<float> *features){
	...
	for(int i = 0; i < Q.size(); i++){
		if(Q[i].size()==1){
			...
			boolvar aux = newBoolVar(*features);
	...
	addAMOPB(Q2,X2,K2,encoding,features);
}




literal SMTFormula::assertMDDLEQAbio(MDD * mdd, std::vector<float> *features) {
	...
	for(int i = 0; i <= mdd->getId(); i++)
		asserted[i] = undef; 
	return assertMDDLEQAbio(mdd,asserted,features);
}

literal SMTFormula::assertMDDLEQAbio(MDD * mdd, std::vector<literal> & asserted, std::vector<float> *features) {
	if(mdd->isTrueMDD())
		return trueVar(features);
	else if(mdd->isFalseMDD())
		return falseVar(features);

	literal v=asserted[mdd->getId()];
	if(v.v.id==-1){
		v = newBoolVar(*features);
		asserted[mdd->getId()]=v;

		literal velse = assertMDDLEQAbio(mdd->getElseChild(),asserted,features);
		addClause(velse | !v);
		...
		for(int i = 0; i < mdd->getNSelectors();i++){
			...
			{
				literal vi = assertMDDLEQAbio(p.second,asserted,features);
...
void SMTFormula::addPBLIA(const std::vector<int> & Q, const std::vector<literal> & X, int K, std::vector<float> *features){
...
void SMTFormula::addAMKSequentialCounter(const std::vector<literal> & x, int K, std::vector<float> *features){
...
void SMTFormula::addAMOPBSWC(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features){
...
void SMTFormula::addAMOPBSorter(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features){
	...
	for(int i = 0; i < n; i++){
		...
		if(maxq > 0){
			...
			addOrderEncoding(maxq,order,features);
			for(int j = 0; j < Q[i].size(); j++)
				if(Q[i][j]>0)
...
void SMTFormula::addAMOPBGeneralizedTotalizer(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features){
...
void SMTFormula::addAMOPBModuloTotalizer(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features){
	...
	nLevelsMTO(Q,X,0,Q.size(),moduli,D,features);
...
void SMTFormula::nLevelsMTO(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int lIndex, int partSize, 
	const std::vector<int> & moduli, std::vector<std::map<int,literal> > & D, std::vector<float> *features){
	...
	//Recursive case, branch in the binary tree
		...
		int lSize = m/2;
		int rSize = m - m/2;
		nLevelsMTO(Q,X,lIndex, lSize, moduli,  Dleft, features);
		nLevelsMTO(Q,X,lIndex+lSize, rSize, moduli, Dright, features);
		...
		for(int h = 0; h < n; h++){
			...
			D[h][0] = trueVar(features);
			c[h] = newBoolVar(*features);
			...
			for(i = Dleft[h].begin(); i != Dleft[h].end(); i++){
				for(j = Dright[h].begin(); j != Dright[h].end(); j++){
					...
					if(sum>0){ //Clauses for sum==0 trivially satisfied
						...
						if(sum<moduli[h])
							addClause(base | MTOVar(D[h],sum, features) | c[h]);
						else{
							thisLevelHasCarry=true;
							addClause(base | c[h]);
							if(sum > moduli[h] && sum%moduli[h]>0)
								addClause(base | MTOVar(D[h],sum%moduli[h],features));
						}
					}
					...
					if(h>0 && prevLevelHasCarry){ //If previous level never has carry, clauses trivially satisfied
						...
						if(sum < moduli[h])
							addClause(base | !c[h-1] | MTOVar(D[h],sum, features) | c[h]);
						else {
							thisLevelHasCarry=true;
							addClause(base | !c[h-1] | c[h]);
							if(sum > moduli[h] && sum%moduli[h]>0)
								addClause(base | !c[h-1] | MTOVar(D[h],sum%moduli[h], features));
						}
					}
				}
			}		
		...
		for(i = Dleft[n].begin(); i != Dleft[n].end(); i++){
			for(j = Dright[n].begin(); j != Dright[n].end(); j++){
				...
				if(sum!=0)
					addClause(base | MTOVar(D[n],sum,features));
				
				if(prevLevelHasCarry)
					addClause(base | !c[n-1] | MTOVar(D[n],sum+1,features));
			}
		}
	}
...
literal SMTFormula::MTOVar(std::map<int,literal> & m, int w, std::vector<float> *features){
	if(m.find(w)==m.end()){
		boolvar x = newBoolVar(*features);
		m[w]=x;
		return x;
	}
	else
...
void SMTFormula::addAMOPBLocalPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features)
{
	int n = Q.size();
...
literal SMTFormula::addPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, bool useSorter, std::vector<float> *features){
	int n = Q.size();
	...
	for(int i = 0; i <= p; i++){
		...
		if(i==0)
			...
			if(useSorter)
				addMerge(U,Shalf,S,true,false,features);
			else
				addQuadraticMerge(U,Shalf,S);
		}
		if(util::nthBit(T,i))
			S.insert(S.begin(),trueVar(features));
...
void SMTFormula::addAMOPBGlobalPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, bool useSorter, std::vector<float> *features){
	if(K==0){
		for(const std::vector<literal> & v : X)
			for(const literal & l : v)
				addClause(!l);
	...
	addClause(!addPolynomialWatchdog(Q,X,K,useSorter,features));
}

void SMTFormula::addAMOPBNaiveLocalPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, bool useSorter, std::vector<float> *features){
	if(K==0){
		for(const std::vector<literal> & v : X)
			for(const literal & l : v)
				addClause(!l);
	...
	for(int i = 0; i < n; i++){
		...
		for(int j = 0; j < Q[i].size(); j++){
			...
			if(watchdog.find(Q[i][j]) == watchdog.end())
				watchdog[Q[i][j]] = addPolynomialWatchdog(Q2,X2,K-Q[i][j],useSorter,features);
			addClause(!watchdog[Q[i][j]] | !X[i][j]);
		}
	}
}

void SMTFormula::addAMOPBReducedGeneralizedGeneralizedTotalizer(std::vector<std::vector<int> >  Q, std::vector<std::vector<literal> > X, int K, bool reduce, std::vector<float> *features){
    ...
    for(int i=n; i<tree_size; i++) {
        ...
        tree[i].literals.push_back(trueVar(features));
    ...
    for(int i=0; i<n; i++) {
        ...
        for(int j=0; j<l->intervals.size(); j++) {
            if(l->intervals[j].first==0) {
                l->literals.push_back(trueVar(features));
            }
            else {
...
void SMTFormula::addOrderEncoding(int x, std::vector<literal> & lits, std::vector<float> *features){

	lits.resize(x);
	lits[0]=newBoolVar(*features);
	for(int i = 1; i < x; i++){
		lits[i] = newBoolVar(*features);
...
void SMTFormula::addTwoComparator(const literal &x1, const literal &x2, literal &y1, literal &y2, bool leqclauses, bool geqclauses, std::vector<float> *features){
	
	y1 = newBoolVar(*features);
	y2 = newBoolVar(*features);
...
void SMTFormula::addQuadraticMerge(const std::vector<literal> &x1, const std::vector<literal> &x2, std::vector<literal> &y, std::vector<float> *features){
	if(x1.empty())
		y = x2;
	else if(x2.empty())
		y = x1;
...
void SMTFormula::addMerge(const std::vector<literal> &x1, const std::vector<literal> &x2, std::vector<literal> &y, bool leqclauses, bool geqclauses, std::vector<float> *features){
	int a = x1.size();
	int b = x2.size();
	...
	if(a==1 && b==1)
		addTwoComparator(x1[0],x2[0],y[0],y[1],leqclauses,geqclauses,features);
	else if(a == 0)
		y = x2;
	else if(b == 0)
		y = x1;
		...
		addMerge(x1even, x2even, zeven,leqclauses,geqclauses,features);
		addMerge(x1odd, x2odd,zodd,leqclauses,geqclauses,features);
		...
		if(a%2==0){
			if(b%2==0){
				...
				for(int i = 1; i < a+b-2; i+=2)
					addTwoComparator(z[i],z[i+1],y[i],y[i+1],leqclauses,geqclauses,features);
			...
			if(b%2==0)
				addMerge(x2,x1,y,leqclauses,geqclauses,features);
			else{
...
void SMTFormula::addSimplifiedMerge(const std::vector<literal> &x1, const std::vector<literal> &x2, std::vector<literal> &y, int c, bool leqclauses, bool geqclauses, std::vector<float> *features){
	int a = x1.size();
	int b = x2.size();
	...
	if(a==1 && b==1 && c==1){
		...
		addSimplifiedMerge(std::vector<literal>(x1.begin(),x1.begin()+c),x2,y,c,leqclauses,geqclauses,features);
	else if(b > c)
		addSimplifiedMerge(x1,std::vector<literal>(x2.begin(),x2.begin()+c),y,c,leqclauses,geqclauses,features);
	else if(a+b<=c)
		addMerge(x1,x2,y,leqclauses,geqclauses,features);
	else{
		y.resize(c);
		std::vector<literal> x1even, x1odd, x2even, x2odd;
		for(int i = 0; i < a-1; i+=2){
		...
		if(c%2==0){
			addSimplifiedMerge(x1even, x2even, zeven, c/2 + 1,leqclauses,geqclauses,features);
			addSimplifiedMerge(x1odd, x2odd, zodd, c/2,leqclauses,geqclauses,features);
			...
			addSimplifiedMerge(x1even, x2even, zeven, (c+1)/2,leqclauses,geqclauses,features);
			addSimplifiedMerge(x1odd, x2odd, zodd, (c-1)/2,leqclauses,geqclauses,features);
...
void SMTFormula::addTotalizer(const std::vector<literal> &x, const std::vector<std::pair<int,std::set<int> > > &inputBits, std::vector<literal> &y, int lIndex, 
	...
	std::map<std::set<std::pair<int,int> >,  std::vector<literal> > & constructed, std::vector<float> *features){
	...
	//Recursive case, branch in the binary tree
		...
		if(lSize==1 && ommittedLeaf==lIndex){
			addTotalizer(x, inputBits, y, rlIndex, rSize,ommittedLeaf, constructed, features);
			return;
		}
		//If right child not present, return left child
		else if(rSize==1 && ommittedLeaf==rlIndex){
			addTotalizer(x, inputBits, y, lIndex, lSize,ommittedLeaf, constructed, features);
			return;
		}
		...
		addTotalizer(x, inputBits, yleft, lIndex, lSize,ommittedLeaf, constructed, features);
		addTotalizer(x, inputBits, yright, rlIndex, rSize,ommittedLeaf, constructed, features);
...
void SMTFormula::addTotalizer(const std::vector<literal> &x, std::vector<literal> &y, std::vector<float> *features){
...
void SMTFormula::addTotalizer(const std::vector<literal> &x, std::vector<literal> &y, int K, std::vector<float> *features){
...
void SMTFormula::addSorting(const std::vector<literal> &x, std::vector<literal> &y, bool leqclauses, bool geqclauses, std::vector<float> *features){
	...
	if(n==1)
		...
		y.resize(2);
		addTwoComparator(x[0],x[1],y[0],y[1],leqclauses,geqclauses, features);
	}
	else{
		...
		addSorting(x1,z1,leqclauses,geqclauses,features);
		addSorting(x2,z2,leqclauses,geqclauses,features);
		addMerge(z1,z2,y,leqclauses,geqclauses,features);
	}
}


void SMTFormula::addMCardinality(const std::vector<literal> &x, std::vector<literal> &y, int m, bool leqclauses, bool geqclauses, std::vector<float> *features){
	//Codifies a mergesort
	int n = x.size();
	...
	if(n<=m){
		addSorting(x,y,leqclauses,geqclauses,features);
		return;
	}
	...
	addMCardinality(x1,z1,std::min(n/2,m),leqclauses,geqclauses,features);
	addMCardinality(x2,z2,std::min(n-(n/2),m),leqclauses,geqclauses,features);
	addSimplifiedMerge(z1,z2,y,m,leqclauses,geqclauses,features);
}
...
void SMTFormula::addGerarquicAMOAMK(const std::vector<std::vector<std::vector<int> > > &Q,
		...
		const std::vector<int> & capacities, std::vector<float> *features){
	...
	if(n==1){
		addAMOPB(Q[0],X[0],capacities[0],AMOPB_GSWC,features);
		return;
	}
	...
	for(int i = n-1; i >= 0; i--)
		addAMOPBCardialitySWC(Q[i],X[i],sorted[i],cumulcap[i],features);
	...
	for(int i = n-1; i >= 0; i--){
		...
		if(merged.size() > capacities[i]){
			...
			addQuadraticMergeCardinality(merge_linput,sorted[i],merged,cumulcap[i],features);
		}
...
void SMTFormula::addAMOPBCardialitySWC(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, std::vector<literal> & output, int K, std::vector<float> *features){
...
void SMTFormula::addQuadraticMergeCardinality(const std::vector<literal> &x1, const std::vector<literal> &x2, std::vector<literal> &y, int K, std::vector<float> *features){
...
boolvar SMTFormula::newBoolVar(const std::vector<float> & features){
	boolvar x = SMTFormula::newBoolVar();
	varsFeatures[varsFeatures.size()-1] = features;
	return x;
}

boolvar SMTFormula::newBoolVar(const std::string & s, const std::vector<float> & features){
	boolvar x = SMTFormula::newBoolVar(s);
	varsFeatures[varsFeatures.size()-1] = features;
	return x;
}

boolvar SMTFormula::newBoolVar(const std::string & s, int i1, const std::vector<float> & features){
	boolvar x = SMTFormula::newBoolVar(s,i1);
	varsFeatures[varsFeatures.size()-1] = features;
	return x;
}

boolvar SMTFormula::newBoolVar(const std::string & s, int i1, int i2, const std::vector<float> & features){
	boolvar x = SMTFormula::newBoolVar(s,i1,i2);
	varsFeatures[varsFeatures.size()-1] = features;
	return x;
}

boolvar SMTFormula::newBoolVar(const std::string & s, int i1, int i2, int i3, const std::vector<float> & features){
	boolvar x = SMTFormula::newBoolVar(s,i1,i2,i3);
	varsFeatures[varsFeatures.size()-1] = features;
	return x;
}

const std::vector<float> & SMTFormula::getFeatures(int i) const{
	return varsFeatures[i];
}

const std::vector<std::vector<float>> & SMTFormula::getVarsFeatures() const {
	return varsFeatures;
}
