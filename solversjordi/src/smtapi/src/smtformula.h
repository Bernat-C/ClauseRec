...
private:
	...
	std::vector<std::vector<float>> varsFeatures; //Map of Boolean variable features
	...
	void addOrderEncoding(int x, std::vector<literal> & lits, std::vector<float> *features = nullptr);

	//Adds the codification of Sorter [x1,x2] -> [y1,y2]
	void addTwoComparator(const literal &x1, const literal &x2, literal &y1, literal &y2, bool leqclauses, bool geqclauses, std::vector<float> *features = nullptr);

	//Adds the codification of "y is the result of merging x1,x2". Used in cardinality constraint
	void addSimplifiedMerge(const std::vector<literal> &x1, const std::vector<literal> &x2, std::vector<literal> &y, int c, bool leqclauses, bool geqclauses, std::vector<float> *features = nullptr);

	void addQuadraticMerge(const std::vector<literal> &x1, const std::vector<literal> &x2, std::vector<literal> &y, std::vector<float> *features = nullptr);

	void addTotalizer(const std::vector<literal> &x, std::vector<literal> &y, std::vector<float> *features = nullptr);
	...
	std::map<std::set<std::pair<int,int> >,  std::vector<literal> > & constructed, std::vector<float> *features = nullptr);

	void addTotalizer(const std::vector<literal> &x, std::vector<literal> &y, int k, std::vector<float> *features = nullptr);

	literal assertMDDLEQAbio(MDD * mdd, std::vector<float> *features = nullptr);

	literal assertMDDLEQAbio(MDD * mdd, std::vector<literal> & asserted, std::vector<float> *features = nullptr);

	literal assertMDDGTAbio(MDD * mdd, std::vector<float> *features = nullptr);

	literal assertMDDGTAbio(MDD * mdd, std::vector<literal> & asserted, std::vector<literal> & elses, std::vector<float> *features = nullptr);

	void addPBLIA(const std::vector<int> & Q, const std::vector<literal> & X, int K, std::vector<float> *features = nullptr);

	void addAMKSequentialCounter(const std::vector<literal> & x, int K, std::vector<float> *features = nullptr);

	void addAMOPBSWC(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features = nullptr);

	void addAMOPBSorter(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features = nullptr);

	void addAMOPBGeneralizedTotalizer(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features = nullptr);

	void addAMOPBReducedGeneralizedGeneralizedTotalizer(std::vector<std::vector<int> >  Q, std::vector<std::vector<literal> > X, int K, bool reduce, std::vector<float> *features = nullptr);
	void mergeValues(const std::vector<int> & n1, const std::vector<int> & n2, std::vector<int> & n3, int K);


	

	literal MTOVar(std::map<int,literal> & m, int w, std::vector<float> *features = nullptr);
	...
	void addAMOPBModuloTotalizer(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features = nullptr);


	literal addPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, bool useSorter, std::vector<float> *features = nullptr);

	void addAMOPBLocalPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, std::vector<float> *features = nullptr);

	void addAMOPBNaiveLocalPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, bool useSorter, std::vector<float> *features = nullptr);

	void addAMOPBGlobalPolynomialWatchdog(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, bool useSorter, std::vector<float> *features = nullptr);
...
public:
	...
	const std::vector<int> & moduli, std::vector<std::map<int,literal> > & D, std::vector<float> *features = nullptr);
	...
	boolvar falseVar(std::vector<float> *features = nullptr);

	//Get the trivially true variable
	boolvar trueVar(std::vector<float> *features = nullptr);
	...
	//Get a new named Boolean variable, with up to 3 subindices in the name and its list of features
	boolvar newBoolVar(const std::vector<float> & features);
	boolvar newBoolVar(const std::string & var, const std::vector<float> & features);
	boolvar newBoolVar(const std::string & var, int i1, const std::vector<float> & features);
	boolvar newBoolVar(const std::string & var, int i1, int i2, const std::vector<float> & features);
	boolvar newBoolVar(const std::string & var, int i1, int i2, int i3, const std::vector<float> & features);

	//Methods to recover the list of features
	const std::vector<float> & getFeatures(int i) const;

	const std::vector<std::vector<float>> & getVarsFeatures() const;
	...
	void addAMO(const std::vector<literal> & v, AMOEncoding enc = AMO_QUAD, std::vector<float> *features = nullptr);

	//Adds exactly-one constraint on the literals in 'v'
	void addEO(const std::vector<literal> & v, AMOEncoding enc = AMO_QUAD, std::vector<float> *features = nullptr);

	//Adds at-least-K constraint on the literals in 'v'
	void addALK(const std::vector<literal> & v, int K, CardinalityEncoding enc = CARD_SORTER, std::vector<float> *features = nullptr);

	//Adds at-most-K constraint on the literals in 'v'
	void addAMK(const std::vector<literal> & v, int K, CardinalityEncoding enc = CARD_SORTER, std::vector<float> *features = nullptr);

	//Adds exactly-K constraint on the literals in 'v'
	void addEK(const std::vector<literal> & v, int K, std::vector<float> *features = nullptr);

	//Adds PB constraint Q*X <= K
	void addPB(const std::vector<int> & Q, const std::vector<literal> & X, int K, PBEncoding = PB_BDD, std::vector<float> *features = nullptr);

	//Adds PB constraint Q*X >= K
	void addPBGEQ(const std::vector<int> & Q, const std::vector<literal> & X, int K, PBEncoding = PB_BDD, std::vector<float> *features = nullptr);

	//Adds AMO-PB constraint Q*X <= K
	void addAMOPB(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, AMOPBEncoding encoding = AMOPB_AMOMDD, std::vector<float> *features = nullptr);

	//Adds AMO-PB constraint Q*X >= K
	void addAMOPBGEQ(const std::vector<std::vector<int> > & Q, const std::vector<std::vector<literal> > & X, int K, AMOPBEncoding encoding = AMOPB_AMOMDD, std::vector<float> *features = nullptr);

	//Adds the codification of "y is the x list sorted  decreasingly". Used in cardinality constraint
	void addSorting(const std::vector<literal> &x, std::vector<literal> &y, bool leqclauses, bool geqclauses, std::vector<float> *features = nullptr);
	...
	void addMCardinality(const std::vector<literal> &x, std::vector<literal> &y, int m, bool leqclauses, bool geqclauses, std::vector<float> *features = nullptr);

	//Adds the codification of "y is the result of merging x1,x2". Used in cardinality constraint
	void addMerge(const std::vector<literal> &x1, const std::vector<literal> &x2, std::vector<literal> &y, bool leqclauses, bool geqclauses, std::vector<float> *features = nullptr);

	void addGerarquicAMOAMK(const std::vector<std::vector<std::vector<int> > > &Q,
		const std::vector<std::vector<std::vector<literal> > > & X,
		const std::vector<int> & capacities, std::vector<float> *features = nullptr);

	void addAMOPBCardialitySWC(const std::vector<std::vector<int> > & Q, 
		const std::vector<std::vector<literal> > & X, std::vector<literal> & output, int K, std::vector<float> *features = nullptr);


	void addQuadraticMergeCardinality(const std::vector<literal> &x1, const std::vector<literal> &x2, std::vector<literal> &y, int K, std::vector<float> *features = nullptr);
