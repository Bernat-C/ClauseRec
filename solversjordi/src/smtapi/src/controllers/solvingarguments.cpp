...
SolvingArguments::SolvingArguments() : Arguments<SolvingArg>(
	...
	*/
	...
	arguments::bop("","use-gnn",USE_GNN,false,
	"Wether to use the GNN API")
	},
	""
	)
{
...
Encoder * SolvingArguments::getEncoder(Encoding * enc){
	...
	if(getBoolOption(USE_API)){
		...
		else if(solver=="minisat" || solver=="maple"){
		#ifndef USEMINISAT
			std::cerr << "Error: this binary has been compiled without support for minisat. " << std::endl;
			exit(UNSUPPORTEDFUNC_ERROR);
		#else
			e = new MinisatAPIEncoder(enc,getIntOption(TRACE_SAT),getBoolOption(ENABLE_RESTARTS),getIntOption(PHASE_SAVING),getStringOption(FILE_PREFIX),getBoolOption(USE_GNN));
		#endif
		}
		else{
