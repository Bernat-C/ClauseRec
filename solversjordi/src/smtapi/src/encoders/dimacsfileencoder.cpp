...
std::string DimacsFileEncoder::getCall() const{
	if(produceModels()){
		if(solver == "glucose_release")
			...
			return (solver + " --no-binary " + getTMPPlainFileName() + " " + "proof.drat" + " | grep -E '(^s )|(^v )'");
...
bool DimacsFileEncoder::checkSAT(int lb, int ub){
	...
	std::string filename = getTMPPlainFileName();
	...
	if(!produceModels() && (solver=="glucose_release" || solver=="kissat")){
	...
	if(sat && produceModels()){
		...
		while(i < workingFormula.f->getNBoolVars()){
			if(results[j]!="v"){
		...
		std::cout << "Proved in " + std::to_string(lastchecktime)  + "seconds" << std::endl;
		double elapsed_time_ms = std::chrono::duration<double, std::milli>(t_end-t_start).count();
		std::cout << "Total time: " + std::to_string(elapsed_time_ms/1000)  + "seconds" << std::endl;
	}
	else {
		double elapsed_time_ms = std::chrono::duration<double, std::milli>(t_end-t_start).count();
		std::cout << "UNSAT Proving time: " + std::to_string(elapsed_time_ms/1000)  + "seconds" << std::endl;
	}
	...
	for(const clause & c : f->getClauses()){
		for(const literal & l : c.v){
			...
			if(l.v.id <= 0 || l.v.id>f->getNBoolVars()){
				std::cerr << "Error: asserted undefined Boolean variable : " << l.sign << " " << l.v.id << " NBV " << f->getNBoolVars() << std::endl;
