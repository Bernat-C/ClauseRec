...
bool GlucoseAPIEncoder::assertAndCheck(int lb, int ub, std::vector<literal> * assumptions){
	...
	std::vector<std::vector<float>> features = workingFormula.f->getVarsFeatures();
	assert(features.size() == workingFormula.f->getNBoolVars());
	
	for(int i = lastVar+1; i<features.size(); i++) {
		vec<float> v;
		for(int j =0; j< features[i].size(); j++) // Create vec<float> from vector<float>
			v.push(features[i][j]);
		vars[i]=s->newVar(true, true, v);
	}
