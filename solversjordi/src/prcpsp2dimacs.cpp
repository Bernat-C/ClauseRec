...
bool getSat(SolvingArguments * sargs, PRCPSP * instance){
	bool sat;
	float time;
	PRCPSPSATEncoding * enc = new PRCPSPSATEncoding(instance, sargs->getAMOPBEncoding());
...
int main(int argc, char **argv) {
	...
	PRCPSPEncoding * encoding = new PRCPSPSATEncoding(instance, sargs->getAMOPBEncoding());
	...
	if(pargs->getBoolOption(COMPUTE_UB) && sargs->getIntOption(UPPER_BOUND)== INT_MIN){
		...
		if(sat){ */
			UB = instance->computePSS(starts,en_execucio);
			if(sargs->getBoolOption(PRINT_NOOPTIMAL_SOLUTIONS)){
				...
				cout << "v ";
				instance->printSolution(cout, starts, en_execucio);
				cout << endl << endl;
			}
	...
	if(sargs->getBoolOption(OUTPUT_ENCODING)){
		...
		opti->setOnProvedSAT([=](){BasicController::onProvedSAT();});
		...
		if(opt==INT_MIN) { //If no better solution found than the one found in the greedy heuristic, that is the objective
			opt = UB+1;
			instance->printSolution(std::cout, starts, en_execucio);
			std::cout << std::endl;
